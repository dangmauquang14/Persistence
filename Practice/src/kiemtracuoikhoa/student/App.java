package kiemtracuoikhoa.student;
...
public class App {
	public static void main(String[] args) {
		...
		List<Student> sortStudents = students.stream().sorted(Comparator.comparing(Student::getDtb))
				.collect(Collectors.toList());
		System.out.println(sortStudents);

		System.out.println("2. Viết hàm tìm kiếm danh sách sinh viên có ĐTB > 8");
		List<Student> searchByDtb = students.stream().filter(student -> student.getDtb() > 8)
				.collect(Collectors.toList());
		System.out.println(searchByDtb);

		System.out.println("3. Viết hàm tìm kiếm toàn bộ các sinh viên NỮ");
		List<Student> searchByGender = students.stream().filter(student -> student.getGender() == Gender.Nu)
				.collect(Collectors.toList());
		...
		System.out.println(getUniqueStudents(students));
...
//		Cach 1
//		students.stream().map(Student::getId).distinct().collect(Collectors.toList());
//		return students;
